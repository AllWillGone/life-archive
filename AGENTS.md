...
- agent 主动给出的建议、行动提醒、安慰话术和支持性分析，默认只在 `session/` 的对话摘要中简短保留，不作为自传事实或情绪底层记录。
- `support/` 是可选的长期支持层；只有当使用者明确希望长期回看建议、行动提醒或支持性分析时，才使用 `support/` 单独归档。
...
7. 如果本仓库启用了可选 `support/` 长期支持层，且需要回看建议或行动提醒时，读取 `support/index.md` 和相关 `support/by_event/` 文件
...
- `support/index.md`：可选建议与支持内容索引；未启用长期支持层时可以只保留占位说明。
- `support/by_event/`：可选地按事件归档 agent 建议、行动提醒或支持性分析；这些内容可供回看，但不混入 `feeling/`。
...
- 新建 `fact/events/` 文件时要检查边界：只有当内容有独立对象、独立阶段、持续发展、长期意义，或确实无法归入已有事件时，才保留为独立事件。
- 普通琐事、一次性聊天、短期情绪波动和已有事件的补充，优先并入已有 `fact`、当天 `session` 或 `inbox`，不要因为信息新就自动新建事件。
...
- [相关支持记录](../../support/by_event/同名或对应文件.md)（如果启用了可选 `support/` 长期支持层）
```
...
- 对话中的建议若有保存价值，默认只在 `session/` 的对话摘要中简短保留；只有当使用者明确希望长期保存这类支持内容时，才写入可选 `support/`。
...
- agent 主动给出的建议、行动方案、安慰话术和分析性支持，不默认写入 `feeling/`。
- 不再默认使用“可能有帮助的支持”栏目。只有当这类内容已经明确属于使用者自己的需要、愿望、当时理解或已采纳想法时，才可并入对应栏目；否则应留在 `session/`，或在使用者明确启用长期支持归档时写入 `support/`。
...
如果出现新的 agent 建议、行动提醒、安慰话术或支持性分析：

- 默认只在当天 `session/` 摘要中简短保留，不另行创建目录或索引。
- 普通安慰、临场分析和一次性建议不进入 `fact/`、`feeling/` 或 `summary/`。
- 只有当建议已经变成使用者明确认可、复述、采纳或以“我想 / 我需要 / 我希望”表达的内容时，才可写入 `feeling` 的“需要或愿望”或“当时理解”。
- 如果使用者明确希望长期回看某些建议、行动提醒或支持性分析，可以写入可选 `support/by_event/` 并更新 `support/index.md`。
